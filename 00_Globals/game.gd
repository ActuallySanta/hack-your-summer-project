...
## The save file the game reads and writes. There is no save picker yet, so there is
...
## Skips the menu and starts the game from [member custom_save_data].
@export var use_custom_save := false
## A starting point written by hand, for testing part of the game without playing up
## to it. Only used when [member use_custom_save] is on; with it on and this left
## empty, the game starts from the beginning as usual.
@export var custom_save_data : CustomSaveData
...
## Puts the game into a save and starts playing it.
##
## This is the whole of what [GameManager] knows about saving: which save to ask for,
## and when. What one holds, and how it reaches the disk, is [SaveManager]'s business.
func _load_game(ignore_custom_save := false) -> void:
	_begin_transition()

	if use_custom_save and not ignore_custom_save and custom_save_data:
		SaveManager.load_custom_save(custom_save_data)
		await _enter_world(true)
	elif SaveManager.load_data_from_file(current_save_name):
		await _enter_world(false)
	else:
		# "Load game" with nothing saved yet. Start one rather than refuse -- there is
		# no save picker to grey the option out on.
...
## Brings up the room the loaded save names and puts the player into it.
##
## [param write_starting_save] is for a run beginning somewhere no save station put
## it -- a new game, or a custom save. Writing that start down is what makes dying
## revert to it, rather than to whatever the last playthrough left on disk.
func _enter_world(write_starting_save: bool) -> void:
	_player_positioned = false
	_reset_world_state()
	await load_room(SaveManager.get_spawn_room())
	...
	LevelManager.set_checkpoint(SaveManager.get_spawn_room(), _player.position, false)

	if write_starting_save:
...
## Drops whatever the run that just ended left the world holding, [b]before[/b] the
## first room of the new one goes up.
