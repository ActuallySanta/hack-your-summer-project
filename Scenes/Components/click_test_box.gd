...
func _input(event: InputEvent) -> void:
	if not event is InputEventMouseButton or not event.button_index == MOUSE_BUTTON_LEFT:
		return
...
func _draw() -> void:
	if not show_boundries:
		return
	draw_rect(Rect2(Vector2.ZERO, dimensions), Color.WHITE, false, 0.5)
